...
[`WeaveTracer`][agentlightning.tracer.weave.WeaveTracer] is an experimental tracer that integrates with [Weave Python SDK](https://docs.wandb.ai/weave). It can work as a substitute for [`AgentOpsTracer`][agentlightning.AgentOpsTracer] if the AgentOps SDK is not suitable for your use case.

The instrumentation to all LLM calls and agent libraries are all done by Weave SDK. Different from [`AgentOpsTracer`][agentlightning.AgentOpsTracer], Weave does not rely on OpenTelemetry to export and upload the spans. Instead, it incorporates a dedicated Weave Trace Server. By implementing a customized Weave Trace Server, Agent-lightning is able to capture all the calls generated by Weave SDK and store them into the [`LightningStore`][agentlightning.LightningStore].

!!! warning

    [`WeaveTracer`][agentlightning.tracer.weave.WeaveTracer] is still experimental and we didn't test the tracer fully like we did with [`AgentOpsTracer`][agentlightning.AgentOpsTracer]. For example, the tracer might have conflicts with libraries that have OpenTelemetry instrumentation enabled by default (such as LiteLLM-based LLM proxy). Please use the tracer with caution and report any issues you encounter to the Agent-lightning team.
