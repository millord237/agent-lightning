...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        if custom_reward_mean_std == {}:
            ...
            # 注意这里的reward & std如果是列表，其顺序是线性的；和score一致
            for i in range(bsz):
...
def compute_reinforce_plus_plus_baseline_outcome_advantage(token_level_rewards: torch.Tensor, response_mask: torch.Tensor,\
    index: torch.Tensor, traj_index: np.ndarray, epsilon: float = 1e-6, compute_mean_std_cross_all_data: bool = True):
    """
...
def compute_rloo_outcome_advantage(token_level_rewards: torch.Tensor, response_mask: torch.Tensor,\
    index: np.ndarray, traj_index: np.ndarray, epsilon: float = 1e-6, compute_mean_std_cross_all_data: bool = True):
    """
...
def compute_policy_loss_gpg(old_log_prob, log_prob, advantages, response_mask, loss_agg_mode="token-mean", config=None, is_replay: bool = False):
    """Adapted from
...
def compute_policy_loss_clip_cov(
    ...
    # # if is_replay:
    # #     save_dir = "/cfs_turbo/yuleiqin/VERL/verl-agent/statistics_alfworld_1.5b/replay_buffer"
    # # else:
    # #     save_dir = "/cfs_turbo/yuleiqin/VERL/verl-agent/statistics_alfworld_1.5b/online_batch"

    # # if is_replay:
    # #     save_dir = "/cfs_turbo/yuleiqin/VERL/verl-agent/statistics_webshop_1.5b/replay_buffer"
    # # else:
    # #     save_dir = "/cfs_turbo/yuleiqin/VERL/verl-agent/statistics_webshop_1.5b/online_batch"

    # # if is_replay:
    # #     save_dir = "/cfs_turbo/yuleiqin/VERL/verl-agent/statistics_alfworld_7b/replay_buffer"
    # # else:
    # #     save_dir = "/cfs_turbo/yuleiqin/VERL/verl-agent/statistics_alfworld_7b/online_batch"

    # # if is_replay:
    # #     save_dir = "/cfs_turbo/yuleiqin/VERL/verl-agent/statistics_webshop_7b/replay_buffer"
    # # else:
    # #     save_dir = "/cfs_turbo/yuleiqin/VERL/verl-agent/statistics_webshop_7b/online_batch"
    ...
    # save_filename = os.path.join(save_dir, f"{timestamps}_{random_str}.numpy")
    # cov_all_np = cov_all.detach().cpu().numpy()
    # np.save(save_filename, cov_all_np)
    # # import pdb;pdb.set_trace();
    # ################## only for debug
