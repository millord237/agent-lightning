...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False,\
    norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, gigpo_mode="mean_std_norm",\
...
class RayPPOTrainer:
    ...
    def convert_messages_format(self, messages):
        ...
        for message in messages:
            save_msg = []
            save_dict = {}
            if "tools_available" in message:
                save_dict["tools"] = message["tools_available"]
            ...
            if type(message) is dict:
                messages_candidate = message["messages"]
            elif type(message) is list:
                messages_candidate = message
            else:
                messages_candidate = message.tolist()

            for msg in messages_candidate:
                if type(msg) is dict:
                    if "tool_calls" in msg:
                        tool_calls = []
                        for tool_call_id, tool_call in enumerate(msg["tool_calls"]):
                            try:
                                arguments_str = str2json(tool_call.arguments)
                            except:
                                print("Exceptions occured on tool call functions", tool_call.arguments)
                    ...
                    if msg.tool_calls:
                        ...
                        for tool_call in msg.tool_calls:
                            try:
                                arguments_str = str2json(tool_call.function.arguments)
                            except:
                                print("Exceptions occured on tool call functions", tool_call.function.arguments)
    ...
    def _dump_generations(self, inputs, outputs, scores, reward_extra_infos_dict, ground_truth_scores, dump_path):
        """Dump rollout/validation samples as JSONL."""
        os.makedirs(dump_path, exist_ok=True)
        filename = os.path.join(dump_path, f"{self.global_steps}.jsonl")
        # type(inputs): list of string
        # type(outputs): list of string
        # type(scores): list of float
        # type(reward_extra_infos_dict): dict ['reward', 'is_incomplete', 'is_overlong', 'is_repetitive', 'is_unreadable']
        # type(ground_truth_scores): list of numpy.ndarray
        n = len(inputs)
        base_data = {
            "input": inputs,
            "output": outputs,
        ...
        with open(filename, "w") as f:
            for i in range(n):
                ...
                if type(entry["groundtruth"]) is np.ndarray:
                    image_array = entry.pop("groundtruth")
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            ## 这里的input_ids并没有存储完整信息; 仅存储128个样本
            ...
            # keys: ['rollout_log_probs', 'prompts', 'responses', 'position_ids', 'input_ids', 'attention_mask']

            prompt_ids = test_output_gen_batch.batch['prompts']
            input_ids_wo_padding = [[token_id for token_id in ids if token_id != self.tokenizer.pad_token_id] for ids in prompt_ids]
            ...
            test_batch.meta_info["use_toolcall_reward"] = self.config.algorithm.use_toolcall_reward
            test_batch.meta_info["max_toolcall_steps"] = self.config.algorithm.max_toolcall_steps
            ...
            # import pdb;pdb.set_trace();
            ground_truths = test_batch.non_tensor_batch["anchor_obs"]
        ...
        if val_data_dir:
            self._dump_generations(
                ...
                ground_truth_scores=sample_groundtruth,
    ...
    def _filter_rollout(self, batch, reward_tensor, reward_extra_infos_dict):
        ...
        num_groups, group_size = self.config.data.train_batch_size, self.config.actor_rollout_ref.rollout.n
        # 这里所有的batch等都是按照uid * traj_id来负责的
        # 如果要计算uid下不同group的reward 需要额外计算
        # [55, 58, 55, 59, 52, 57, 60, 61] 每个轨迹拆出来的样本数目其实是不等的
        ...
        in_group_mean = np.mean([item[1] for item in reward_by_uid_mean])
        if rollout_filter_ratio == 1:
            return batch, {"rollout/in_group_std": in_group_std, "rollout/in_group_max": in_group_max, "rollout/in_group_mean": in_group_mean,\
                "rollout/chosen_in_group_std": in_group_std, "rollout/chosen_in_group_max": in_group_max, "rollout/chosen_in_group_mean": in_group_mean}, reward_tensor, reward_extra_infos_dict

        # import pdb;pdb.set_trace();
        num_rollout_filter = int(float(rollout_filter_ratio) * float(len(reward_by_uid_traj_uid)))
        assert(num_rollout_filter > 0)
        if self.config.actor_rollout_ref.rollout.rollout_filter_type == "std_rev":
            std_chosen = sorted(reward_by_uid_std, key=lambda x:x[1])[:num_rollout_filter]
            top_groups = [item[0] for item in std_chosen]
            mean_chosen = [item for item in reward_by_uid_mean if item[0] in top_groups]
            assert(len(std_chosen) == len(mean_chosen))

        elif self.config.actor_rollout_ref.rollout.rollout_filter_type == "std":
            std_chosen = sorted(reward_by_uid_std, key=lambda x:x[1], reverse=True)[:num_rollout_filter]
            top_groups = [item[0] for item in std_chosen]
            mean_chosen = [item for item in reward_by_uid_mean if item[0] in top_groups]
            assert(len(std_chosen) == len(mean_chosen))
        ...
        mask = torch.zeros(len(reward_tensor), dtype=torch.bool)
        valid_idxs = []
        for idx in range(len(reward_tensor)):
            if idx2uid[idx] in top_groups:
                mask[idx] = True
        ...
        def max_multiple_of_8(x):
            return (x // 8) * 8
        # num_rollout_valid_traj = int(max_multiple_of_8(len(valid_idxs)))
        num_rollout_valid_traj = int(nearest_lower_power_of_2(len(valid_idxs)))
        ...
        metrics_update = {
            "rollout/in_group_std": in_group_std,
            "rollout/in_group_max": in_group_max,
            "rollout/in_group_mean": in_group_mean,
            "rollout/chosen_in_group_std": np.mean(std_chosen_values),
            "rollout/chosen_in_group_max": max(mean_chosen_values),
            "rollout/chosen_in_group_mean": np.mean(mean_chosen_values),
        }
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                gen_batch.meta_info["rollout_data_dir"] = rollout_data_dir
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer("update_actor", timing_raw):
                            batch_sft: DataProto = DataProto.from_single_dict({})
                            batch_replay: DataProto = DataProto.from_single_dict({})
                            batch.meta_info["multi_turn"] = self.config.actor_rollout_ref.rollout.multi_turn.enable
                            # 这里update actor
                            actor_output = self.actor_rollout_wg.update_actor(batch, batch_sft, batch_replay)
