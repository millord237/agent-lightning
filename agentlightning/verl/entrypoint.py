...
class TaskRunner:
    def run(
        ...
        if "custom_cls" in config.data and config.data.custom_cls.get("path", None) is not None:
            # Dynamically load the custom dataset class from the specified path
            ...
            print(f"Using custom dataset class: {dataset_cls.__name__} from {config.data.custom_cls.path}")
        else:
            # Use the default AgentDataset class
