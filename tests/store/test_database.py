...
"""
Comprehensive tests for DatabaseStore.
...
from agentlightning.store import DatabaseLightningStore
...
# Test ORM representation and database interactions

# Core CRUD Operations Tests


@pytest.mark.asyncio
async def test_enqueue_rollout_creates_rollout(db_store: DatabaseLightningStore) -> None:
    """Test that enqueue_rollout creates a properly initialized rollout."""
    sample = {"input": "test_data"}
    metadata = {"key": "value", "number": 42}

    rollout = await db_store.enqueue_rollout(
...
@pytest.mark.asyncio
async def test_enqueue_rollout_accepts_config(db_store: DatabaseLightningStore) -> None:
    ...
    rollout = await db_store.enqueue_rollout(input={"sample": True}, config=config)
    ...
    stored = await db_store.get_rollout_by_id(rollout.rollout_id)
...
@pytest.mark.asyncio
async def test_add_rollout_initializes_attempt(db_store: DatabaseLightningStore) -> None:
    """Test that add_rollout immediately tracks a preparing attempt."""
    sample = {"payload": "value"}

    attempt_rollout = await db_store.start_rollout(input=sample, mode="val", resources_id="res-add")
    ...
    stored = await db_store.query_rollouts(status=["preparing"])
    assert len(stored) == 1
    assert stored[0].rollout_id == attempt_rollout.rollout_id
    assert stored[0].resources_id == "res-add"

    attempts = await db_store.query_attempts(attempt_rollout.rollout_id)
    assert len(attempts) == 1
    assert attempts[0].attempt_id == attempt_rollout.attempt.attempt_id

    latest_attempt = await db_store.get_latest_attempt(attempt_rollout.rollout_id)
    assert latest_attempt is not None
    assert latest_attempt.attempt_id == attempt_rollout.attempt.attempt_id


@pytest.mark.asyncio
async def test_start_rollout_accepts_config(db_store: DatabaseLightningStore) -> None:
    ...
    attempt_rollout = await db_store.start_rollout(input={"payload": "value"}, config=config)
    ...
    stored = await db_store.get_rollout_by_id(attempt_rollout.rollout_id)
    assert stored is not None
...
@pytest.mark.asyncio
async def test_query_rollouts_by_status(db_store: DatabaseLightningStore) -> None:
    """Test querying rollouts filtered by status."""
    # Create rollouts with different statuses
    r1 = await db_store.enqueue_rollout(input={"id": 1})
    r2 = await db_store.enqueue_rollout(input={"id": 2})
    r3 = await db_store.enqueue_rollout(input={"id": 3})

    # Modify statuses
    await db_store.dequeue_rollout()  # r1 becomes "preparing"
    await db_store.update_rollout(rollout_id=r2.rollout_id, status="failed")
    # r3 remains "queuing"

    # Test various queries
    all_rollouts = await db_store.query_rollouts()
    assert len(all_rollouts) == 3

    queuing = await db_store.query_rollouts(status=["queuing"])
    assert len(queuing) == 1
    assert queuing[0].rollout_id == r3.rollout_id

    preparing = await db_store.query_rollouts(status=["preparing"])
    assert len(preparing) == 1
    assert preparing[0].rollout_id == r1.rollout_id

    finished = await db_store.query_rollouts(status=["failed", "succeeded"])
    assert len(finished) == 1
    assert finished[0].rollout_id == r2.rollout_id

    # Empty status list
    none = await db_store.query_rollouts(status=[])
    assert len(none) == 0


@pytest.mark.asyncio
async def test_get_rollout_by_id(db_store: DatabaseLightningStore) -> None:
    """Test retrieving rollouts by their ID."""
    # Test getting non-existent rollout
    rollout = await db_store.get_rollout_by_id("nonexistent")
    assert rollout is None

    # Create a rollout
    created = await db_store.enqueue_rollout(input={"test": "data"}, mode="train")

    # Retrieve by ID
    retrieved = await db_store.get_rollout_by_id(created.rollout_id)
    ...
    await db_store.update_rollout(rollout_id=created.rollout_id, status="running")
    updated = await db_store.get_rollout_by_id(created.rollout_id)
...
async def test_store_lock_rebinds_to_new_event_loop(
    db_store: DatabaseLightningStore,
) -> None:
    """The in-memory store can be reused after switching to a new event loop."""

    rollout = await db_store.enqueue_rollout(input={"foo": "bar"})

    def run_in_new_loop() -> Optional[Rollout]:
        loop = asyncio.new_event_loop()
        try:
            return loop.run_until_complete(db_store.get_rollout_by_id(rollout.rollout_id))
        finally:
            loop.close()
...
@pytest.mark.asyncio
async def test_query_rollouts_by_rollout_ids(db_store: DatabaseLightningStore) -> None:
    ...
    selected = await db_store.query_rollouts(rollout_ids=[r1.rollout_id, r3.rollout_id])
    assert len(selected) == 2
    selected_ids = {r.rollout_id for r in selected}
    assert selected_ids == {r1.rollout_id, r3.rollout_id}

    # Query by single ID
    single = await db_store.query_rollouts(rollout_ids=[r2.rollout_id])
    assert len(single) == 1
    assert single[0].rollout_id == r2.rollout_id

    # Query by non-existent ID
    none = await db_store.query_rollouts(rollout_ids=["nonexistent"])
    assert len(none) == 0

    # Combine with status filter
    await db_store.update_rollout(rollout_id=r1.rollout_id, status="succeeded")
    await db_store.update_rollout(rollout_id=r2.rollout_id, status="failed")

    filtered = await db_store.query_rollouts(
...
@pytest.mark.asyncio
async def test_update_rollout_fields(db_store: DatabaseLightningStore) -> None:
    """Test updating various rollout fields."""
    rollout = await db_store.enqueue_rollout(input={"test": "data"})
    ...
    # Verify all updates
    updated_rollouts = await db_store.query_rollouts()
...
@pytest.mark.asyncio
async def test_rollout_config_functionality(db_store: DatabaseLightningStore) -> None:
    ...
    rollout = await db_store.enqueue_rollout(input={"test": "retry"})
    await db_store.update_rollout(rollout_id=rollout.rollout_id, config=config)
    ...
    rollout2 = await db_store.enqueue_rollout(input={"test": "different_config"})
    await db_store.update_rollout(rollout_id=rollout2.rollout_id, config=config2)

    stored2 = await db_store.get_rollout_by_id(rollout2.rollout_id)
    ...
    stored1_again = await db_store.get_rollout_by_id(rollout.rollout_id)
...
@pytest.mark.asyncio
async def test_dequeue_rollout_skips_non_queuing_status(db_store: DatabaseLightningStore) -> None:
    ...
    popped = await db_store.dequeue_rollout()
    ...
    popped2 = await db_store.dequeue_rollout()
...
@pytest.mark.asyncio
async def test_fifo_ordering(db_store: DatabaseLightningStore) -> None:
    """Test that queue maintains FIFO order."""
    rollouts: List[Rollout] = []
    for i in range(5):
        r = await db_store.enqueue_rollout(input={"order": i})
        rollouts.append(r)
...
@pytest.mark.asyncio
async def test_pop_empty_queue(db_store: DatabaseLightningStore) -> None:
    """Test popping from empty queue returns None."""
    result = await db_store.dequeue_rollout()
...
@pytest.mark.asyncio
async def test_requeue_mechanism(db_store: DatabaseLightningStore) -> None:
    """Test requeuing puts rollout back in queue."""
    rollout = await db_store.enqueue_rollout(input={"data": "test"})
    ...
    # Requeue it
    await db_store.update_rollout(rollout_id=original_id, status="requeuing")

    # Should be back in queue
    requeued = await db_store.dequeue_rollout()
    ...
    attempts = await db_store.query_attempts(requeued.rollout_id)
    assert len(attempts) == 2  # First attempt plus requeued attempt

    latest_attempt = await db_store.get_latest_attempt(requeued.rollout_id)
    assert latest_attempt is not None
...
@pytest.mark.asyncio
async def test_add_resources_generates_id_and_stores(db_store: DatabaseLightningStore) -> None:
    """Test that add_resources generates a resources_id and stores the resources."""
    # Initially no resources
    assert await db_store.get_latest_resources() is None
    ...
    resources_update = await db_store.add_resources({"main_llm": llm, "greeting": prompt})
    ...
    latest = await db_store.get_latest_resources()
    ...
    retrieved = await db_store.get_resources_by_id(resources_update.resources_id)
    assert retrieved is not None
    assert retrieved.resources_id == resources_update.resources_id


@pytest.mark.asyncio
async def test_add_resources_multiple_times_generates_unique_ids(db_store: DatabaseLightningStore) -> None:
    ...
    update1 = await db_store.add_resources({"llm": llm1})
    update2 = await db_store.add_resources({"llm": llm2})
    ...
    retrieved1 = await db_store.get_resources_by_id(update1.resources_id)
    retrieved2 = await db_store.get_resources_by_id(update2.resources_id)
...
@pytest.mark.asyncio
async def test_resource_lifecycle(db_store: DatabaseLightningStore) -> None:
    """Test adding, updating, and retrieving resources."""
    # Initially no resources
    assert await db_store.get_latest_resources() is None
    assert await db_store.get_resources_by_id("any-id") is None
    ...
    update = await db_store.update_resources("v1", {"main_llm": llm_v1})
    ...
    v2 = await db_store.update_resources("v2", {"main_llm": llm_v2})
    ...
    old = await db_store.get_resources_by_id("v1")
...
@pytest.mark.asyncio
async def test_task_inherits_latest_resources(db_store: DatabaseLightningStore) -> None:
    ...
    await db_store.update_resources(update.resources_id, update.resources)
    ...
    r2 = await db_store.enqueue_rollout(input={"id": 2}, resources_id="override")
    ...
    await db_store.update_resources(update2.resources_id, update2.resources)
...
@pytest.mark.asyncio
async def test_span_sequence_generation(db_store: DatabaseLightningStore, mock_readable_span: Mock) -> None:
    """Test automatic sequence ID generation for spans."""
    rollout = await db_store.enqueue_rollout(input={"test": "data"})
    # Pop to create an attempt
    await db_store.dequeue_rollout()
    attempts = await db_store.query_attempts(rollout.rollout_id)
    attempt_id = attempts[0].attempt_id

    # First span gets sequence_id 1
    seq_id = await db_store.get_next_span_sequence_id(rollout.rollout_id, attempt_id)
    assert seq_id == 1

    span1 = await db_store.add_otel_span(rollout.rollout_id, attempt_id, mock_readable_span)
    ...
    span2 = await db_store.add_otel_span(rollout.rollout_id, attempt_id, mock_readable_span)
    assert span2.sequence_id == 4

    # FIXME Different attempt reuses the same rollout_id
    seq_id = await db_store.get_next_span_sequence_id(rollout.rollout_id, "attempt-does-not-exist")
    assert seq_id == 5


@pytest.mark.asyncio
async def test_span_with_explicit_sequence_id(db_store: DatabaseLightningStore, mock_readable_span: Mock) -> None:
    ...
    span = await db_store.add_otel_span(rollout.rollout_id, attempt_id, mock_readable_span, sequence_id=100)
    assert span.sequence_id == 100

    next_seq = await db_store.get_next_span_sequence_id(rollout.rollout_id, attempt_id)
    assert next_seq == 101


@pytest.mark.asyncio
async def test_query_spans_by_attempt(db_store: DatabaseLightningStore, mock_readable_span: Mock) -> None:
    ...
    for _ in range(2):
        await db_store.add_otel_span(rollout.rollout_id, attempt1_id, mock_readable_span)

    # Simulate requeue and create second attempt
    await db_store.update_rollout(rollout_id=rollout.rollout_id, status="requeuing")
    await db_store.dequeue_rollout()
    attempts = await db_store.query_attempts(rollout.rollout_id)
    ...
    for _ in range(3):
        await db_store.add_otel_span(rollout.rollout_id, attempt2_id, mock_readable_span)

    # Query all spans
    all_spans = await db_store.query_spans(rollout.rollout_id)
    assert len(all_spans) == 5

    # Query specific attempt
    attempt1_spans = await db_store.query_spans(rollout.rollout_id, attempt_id=attempt1_id)
    assert len(attempt1_spans) == 2
    assert all(s.attempt_id == attempt1_id for s in attempt1_spans)

    # Query latest attempt
    latest_spans = await db_store.query_spans(rollout.rollout_id, attempt_id="latest")
    assert len(latest_spans) == 3
    assert all(s.attempt_id == attempt2_id for s in latest_spans)

    # Query non-existent attempt
    no_spans = await db_store.query_spans(rollout.rollout_id, attempt_id="nonexistent")
...
async def test_span_triggers_status_transition(
    db_store: DatabaseLightningStore, mock_readable_span: Mock
) -> None:
    ...
    rollouts = await db_store.query_rollouts(status=["preparing"])
    ...
    # Add first span
    await db_store.add_otel_span(rollout.rollout_id, attempt_id, mock_readable_span)

    # Status should transition to running
    rollouts = await db_store.query_rollouts(status=["running"])
    assert len(rollouts) == 1
...
async def test_span_does_not_reset_timeout_attempt(
    ...
    rollout = await db_store.enqueue_rollout(input={"test": "timeout-span"})

    # Create the first attempt
    dequeued = await db_store.dequeue_rollout()
    ...
    attempts_before = await db_store.query_attempts(rollout.rollout_id)
    assert attempts_before[0].status == "timeout"

    rollout_before = await db_store.get_rollout_by_id(rollout.rollout_id)
    ...
    attempts_after = await db_store.query_attempts(rollout.rollout_id)
    assert attempts_after[0].status == "timeout"
    assert attempts_after[0].last_heartbeat_time is not None

    rollout_after = await db_store.get_rollout_by_id(rollout.rollout_id)
    assert rollout_after is not None
    assert rollout_after.status == rollout_before.status


@pytest.mark.asyncio
async def test_completion_sets_end_time(db_store: DatabaseLightningStore) -> None:
    ...
    await db_store.update_rollout(rollout_id=rollout.rollout_id, status="succeeded")

    completed_rollouts = await db_store.query_rollouts()
...
@pytest.mark.asyncio
async def test_wait_for_rollouts(db_store: DatabaseLightningStore) -> None:
    """Test waiting for rollout completion."""
    # Add multiple rollouts
    r1 = await db_store.enqueue_rollout(input={"id": 1})
    r2 = await db_store.enqueue_rollout(input={"id": 2})
    _r3 = await db_store.enqueue_rollout(input={"id": 3})

    # Start waiting for r1 and r2
    async def wait_for_completion() -> List[Rollout]:
        return await db_store.wait_for_rollouts(rollout_ids=[r1.rollout_id, r2.rollout_id], timeout=5.0)
...
@pytest.mark.asyncio
async def test_wait_timeout(db_store: DatabaseLightningStore) -> None:
    """Test wait_for_rollouts timeout behavior."""
    rollout = await db_store.enqueue_rollout(input={"test": "data"})

    start = time.time()
    completed = await db_store.wait_for_rollouts(rollout_ids=[rollout.rollout_id], timeout=0.1)
...
@pytest.mark.asyncio
async def test_wait_with_timeout_none_polling(db_store: DatabaseLightningStore) -> None:
    """Test wait_for_rollouts with timeout=None uses polling and can be cancelled."""
    rollout = await db_store.enqueue_rollout(input={"test": "indefinite"})

    async def wait_indefinitely():
        return await db_store.wait_for_rollouts(rollout_ids=[rollout.rollout_id], timeout=None)
...
@pytest.mark.asyncio
async def test_wait_with_timeout_none_can_be_cancelled(db_store: DatabaseLightningStore) -> None:
    """Test that wait_for_rollouts with timeout=None can be cancelled cleanly."""
    rollout = await db_store.enqueue_rollout(input={"test": "cancel"})
...
@pytest.mark.asyncio
async def test_wait_with_timeout_zero(db_store: DatabaseLightningStore) -> None:
    """Test wait_for_rollouts with timeout=0 returns immediately."""
    rollout = await db_store.enqueue_rollout(input={"test": "zero"})

    start = time.time()
    completed = await db_store.wait_for_rollouts(rollout_ids=[rollout.rollout_id], timeout=0)
    elapsed = time.time() - start
...
@pytest.mark.asyncio
async def test_wait_with_already_completed_rollout(db_store: DatabaseLightningStore) -> None:
    """Test wait_for_rollouts returns immediately for already completed rollouts."""
    rollout = await db_store.enqueue_rollout(input={"test": "already_done"})
    ...
    start = time.time()
    completed = await db_store.wait_for_rollouts(rollout_ids=[rollout.rollout_id], timeout=5.0)
    elapsed = time.time() - start
...
@pytest.mark.asyncio
async def test_wait_multiple_rollouts_different_completion_times(db_store: DatabaseLightningStore) -> None:
    ...
    async def wait_for_all():
        return await db_store.wait_for_rollouts(
    ...
    await db_store.update_rollout(rollout_id=r2.rollout_id, status="succeeded")

    await asyncio.sleep(0.05)
    await db_store.update_rollout(rollout_id=r1.rollout_id, status="failed")

    await asyncio.sleep(0.05)
    await db_store.update_rollout(rollout_id=r3.rollout_id, status="succeeded")
...
@pytest.mark.asyncio
async def test_wait_partial_completion_on_timeout(db_store: DatabaseLightningStore) -> None:
...
@pytest.mark.asyncio
async def test_wait_concurrent_waiters_on_same_rollout(db_store: DatabaseLightningStore) -> None:
    """Test multiple concurrent waiters on the same rollout."""
    rollout = await db_store.enqueue_rollout(input={"test": "concurrent"})

    async def wait_for_completion():
        return await db_store.wait_for_rollouts(rollout_ids=[rollout.rollout_id], timeout=2.0)
...
@pytest.mark.asyncio
async def test_wait_nonexistent_rollout_with_finite_timeout(db_store: DatabaseLightningStore) -> None:
    """Test waiting for non-existent rollout with finite timeout."""
    start = time.time()
    completed = await db_store.wait_for_rollouts(rollout_ids=["nonexistent"], timeout=0.1)
    elapsed = time.time() - start
...
@pytest.mark.asyncio
async def test_wait_mixed_existing_and_nonexistent_rollouts(db_store: DatabaseLightningStore) -> None:
...
@pytest.mark.asyncio
async def test_wait_event_set_before_wait_starts(db_store: DatabaseLightningStore) -> None:
    """Test that waiting on an already-set event returns immediately."""
    rollout = await db_store.enqueue_rollout(input={"test": "early_complete"})
    ...
    start = time.time()
    completed = await db_store.wait_for_rollouts(rollout_ids=[rollout.rollout_id], timeout=10.0)
    elapsed = time.time() - start
...
@pytest.mark.asyncio
async def test_wait_polling_interval_with_timeout_none(db_store: DatabaseLightningStore) -> None:
    """Test that timeout=None polling doesn't busy-wait (uses reasonable intervals)."""
    rollout = await db_store.enqueue_rollout(input={"test": "polling"})
    ...
    async def wait_and_complete():
        ...
        wait_task = asyncio.create_task(
            db_store.wait_for_rollouts(rollout_ids=[rollout.rollout_id], timeout=None)
        )
...
@pytest.mark.asyncio
async def test_concurrent_task_addition(db_store: DatabaseLightningStore) -> None:
    """Test adding tasks concurrently."""

    async def enqueue_rollout(index: int) -> Rollout:
        return await db_store.enqueue_rollout(input={"index": index})
...
@pytest.mark.asyncio
async def test_concurrent_pop_operations(db_store: DatabaseLightningStore) -> None:
    """Test concurrent popping ensures each rollout is popped once."""
    # Add 20 tasks
    for i in range(20):
        await db_store.enqueue_rollout(input={"index": i})

    async def pop_task() -> Rollout | None:
        return await db_store.dequeue_rollout()
...
@pytest.mark.asyncio
async def test_concurrent_span_additions(db_store: DatabaseLightningStore, mock_readable_span: Mock) -> None:
    """Test concurrent span additions maintain consistency."""
    await db_store.enqueue_rollout(input={"test": "data"})
    rollout = await db_store.dequeue_rollout()  # Create an attempt
    assert rollout is not None

    async def add_span(index: int) -> Span:
        return await db_store.add_otel_span(rollout.rollout_id, rollout.attempt.attempt_id, mock_readable_span)
...
@pytest.mark.asyncio
async def test_concurrent_resource_updates(db_store: DatabaseLightningStore) -> None:
    ...
    for i in range(50):
        res = await db_store.get_resources_by_id(f"v{i}")
...
@pytest.mark.asyncio
async def test_update_nonexistent_rollout(db_store: DatabaseLightningStore) -> None:
    """Test updating non-existent rollout raises error."""
    with pytest.raises(ValueError, match="Rollout nonexistent not found"):
        await db_store.update_rollout(rollout_id="nonexistent", status="failed")


@pytest.mark.asyncio
async def test_add_span_without_rollout(db_store: DatabaseLightningStore, mock_readable_span: Mock) -> None:
    """Test adding span to non-existent rollout raises error."""
    with pytest.raises(ValueError, match="Rollout nonexistent not found"):
        await db_store.add_otel_span("nonexistent", "attempt-1", mock_readable_span)


@pytest.mark.asyncio
async def test_add_span_with_missing_attempt(db_store: DatabaseLightningStore, mock_readable_span: Mock) -> None:
    ...
    with pytest.raises(ValueError, match="Attempt attempt-missing not found"):
        await db_store.add_span(invalid_span)


@pytest.mark.asyncio
async def test_query_empty_spans(db_store: DatabaseLightningStore) -> None:
    """Test querying spans for non-existent rollout returns empty."""
    spans = await db_store.query_spans("nonexistent")
    assert spans == []

    # With attempt_id
    spans = await db_store.query_spans("nonexistent", attempt_id="attempt-1")
    assert spans == []

    # With latest
    spans = await db_store.query_spans("nonexistent", attempt_id="latest")
    assert spans == []


@pytest.mark.asyncio
async def test_query_latest_with_no_spans(db_store: DatabaseLightningStore) -> None:
    """Test querying 'latest' attempt when no spans exist."""
    rollout = await db_store.enqueue_rollout(input={"test": "data"})

    spans = await db_store.query_spans(rollout.rollout_id, attempt_id="latest")
    assert spans == []


@pytest.mark.asyncio
async def test_wait_for_nonexistent_rollout(db_store: DatabaseLightningStore) -> None:
...
@pytest.mark.asyncio
async def test_query_attempts(db_store: DatabaseLightningStore) -> None:
...
@pytest.mark.asyncio
async def test_get_latest_attempt(db_store: DatabaseLightningStore) -> None:
    """Test getting the latest attempt."""
    rollout = await db_store.enqueue_rollout(input={"test": "data"})

    # No attempts initially
    latest = await db_store.get_latest_attempt(rollout.rollout_id)
...
@pytest.mark.asyncio
async def test_update_attempt_fields(db_store: DatabaseLightningStore) -> None:
    ...
    updated = await db_store.update_attempt(
        rollout_id=rollout.rollout_id,
...
@pytest.mark.asyncio
async def test_update_latest_attempt(db_store: DatabaseLightningStore) -> None:
...
@pytest.mark.asyncio
async def test_update_attempt_sets_end_time_for_terminal_status(db_store: DatabaseLightningStore) -> None:
    """Terminal attempt statuses set end_time while in-progress statuses don't."""
    rollout = await db_store.enqueue_rollout(input={"test": "data"})
    await db_store.dequeue_rollout()

    attempt = (await db_store.query_attempts(rollout.rollout_id))[0]
    assert attempt.end_time is None

    running = await db_store.update_attempt(
        rollout_id=rollout.rollout_id,
        attempt_id=attempt.attempt_id,
        status="running",
    )
    ...
    failed = await db_store.update_attempt(
        rollout_id=rollout.rollout_id,
        attempt_id=attempt.attempt_id,
        status="failed",
    )
    ...
    rollout = await db_store.get_rollout_by_id(rollout_id=rollout.rollout_id)
    assert rollout is not None
...
async def test_rollout_retry_lifecycle_updates_statuses(
    ...
    first_attempted = await db_store.dequeue_rollout()
    assert first_attempted is not None
    assert first_attempted.status == "preparing"

    first_attempt = (await db_store.query_attempts(rollout.rollout_id))[0]
    await db_store.add_otel_span(rollout.rollout_id, first_attempt.attempt_id, mock_readable_span)

    # Status should reflect running state after span is recorded
    running_rollout = await db_store.query_rollouts(status=["running"])
    assert running_rollout and running_rollout[0].rollout_id == rollout.rollout_id

    running_attempts = await db_store.query_attempts(rollout.rollout_id)
    assert running_attempts[0].status == "running"

    # Mark first attempt as failed and requeue rollout
    failed_attempt = await db_store.update_attempt(
        rollout_id=rollout.rollout_id,
    ...
    attempts_after_failure = await db_store.query_attempts(rollout.rollout_id)
    assert [a.status for a in attempts_after_failure] == ["failed"]

    retry_attempted = await db_store.dequeue_rollout()
    assert retry_attempted is not None
    assert retry_attempted.status == "preparing"
    assert retry_attempted.attempt.sequence_id == 2

    latest_pre_span = await db_store.get_latest_attempt(rollout.rollout_id)
    assert latest_pre_span is not None and latest_pre_span.sequence_id == 2
    assert latest_pre_span.status == "preparing"

    await db_store.add_otel_span(rollout.rollout_id, retry_attempted.attempt.attempt_id, mock_readable_span)

    latest_running = await db_store.get_latest_attempt(rollout.rollout_id)
    ...
    final_rollout = await db_store.query_rollouts(status=["succeeded"])
    assert final_rollout and final_rollout[0].rollout_id == rollout.rollout_id

    final_attempts = await db_store.query_attempts(rollout.rollout_id)
    assert [a.status for a in final_attempts] == ["failed", "succeeded"]


@pytest.mark.asyncio
async def test_update_nonexistent_attempt(db_store: DatabaseLightningStore) -> None:
    """Test updating non-existent attempt raises error."""
    rollout = await db_store.enqueue_rollout(input={"test": "data"})

    with pytest.raises(ValueError, match="No attempts found"):
        await db_store.update_attempt(rollout_id=rollout.rollout_id, attempt_id="nonexistent", status="failed")
...
@pytest.mark.asyncio
async def test_add_attempt_creates_new_attempt(db_store: DatabaseLightningStore) -> None:
    """Test add_attempt creates a new attempt for existing rollout."""
    # Create a rollout
    rollout = await db_store.enqueue_rollout(input={"test": "data"})

    # Add first manual attempt
    attempted_rollout = await db_store.start_attempt(rollout.rollout_id)
...
@pytest.mark.asyncio
async def test_add_attempt_increments_sequence_id(db_store: DatabaseLightningStore) -> None:
    """Test add_attempt correctly increments sequence_id."""
    # Create a rollout and dequeue to create first attempt
    rollout = await db_store.enqueue_rollout(input={"test": "data"})
    await db_store.dequeue_rollout()  # Creates attempt with sequence_id=1

    # Add second attempt manually
    attempted_rollout2 = await db_store.start_attempt(rollout.rollout_id)
    assert attempted_rollout2.attempt.sequence_id == 2

    # Add third attempt manually
    attempted_rollout3 = await db_store.start_attempt(rollout.rollout_id)
...
@pytest.mark.asyncio
async def test_add_attempt_nonexistent_rollout(db_store: DatabaseLightningStore) -> None:
    """Test add_attempt raises error for nonexistent rollout."""
    with pytest.raises(ValueError, match="Rollout nonexistent not found"):
        await db_store.start_attempt("nonexistent")


@pytest.mark.asyncio
async def test_add_attempt_ignores_max_attempts(db_store: DatabaseLightningStore) -> None:
    ...
    attempt1 = await db_store.start_attempt(rollout.rollout_id)
    attempt2 = await db_store.start_attempt(rollout.rollout_id)
    attempt3 = await db_store.start_attempt(rollout.rollout_id)  # Should succeed despite max_attempts=2
...
@pytest.mark.asyncio
async def test_status_propagation_only_for_latest_attempt(db_store: DatabaseLightningStore) -> None:
    """Test that status changes only propagate to rollout when updating latest attempt."""
    rollout = await db_store.enqueue_rollout(input={"test": "propagation"})

    # Create multiple attempts
    attempt1 = await db_store.start_attempt(rollout.rollout_id)
    _attempt2 = await db_store.start_attempt(rollout.rollout_id)
    attempt3 = await db_store.start_attempt(rollout.rollout_id)  # This is the latest
...
@pytest.mark.asyncio
async def test_status_propagation_with_retry_for_latest_attempt(db_store: DatabaseLightningStore) -> None:
    ...
    # Create multiple attempts
    attempt1 = await db_store.start_attempt(rollout.rollout_id)  # sequence_id=1
    attempt2 = await db_store.start_attempt(rollout.rollout_id)  # sequence_id=2 (latest)
...
@pytest.mark.asyncio
async def test_status_propagation_latest_changes_when_new_attempt_added(db_store: DatabaseLightningStore) -> None:
    """Test that the 'latest attempt' changes as new attempts are added."""
    rollout = await db_store.enqueue_rollout(input={"test": "latest_changes"})
...
@pytest.mark.asyncio
async def test_status_propagation_update_latest_by_reference(db_store: DatabaseLightningStore) -> None:
    """Test status propagation when updating latest attempt using 'latest' reference."""
    rollout = await db_store.enqueue_rollout(input={"test": "latest_ref"})

    # Create multiple attempts
    await db_store.start_attempt(rollout.rollout_id)
    await db_store.start_attempt(rollout.rollout_id)
    attempt3 = await db_store.start_attempt(rollout.rollout_id)  # This is latest

    # Update using "latest" reference
    updated_attempt = await db_store.update_attempt(
        rollout_id=rollout.rollout_id, attempt_id="latest", status="succeeded"
    )
...
@pytest.mark.asyncio
async def test_healthcheck_timeout_behavior(db_store: DatabaseLightningStore, mock_readable_span: Mock) -> None:
    ...
    rollout = await db_store.enqueue_rollout(input={"test": "timeout"})
    await db_store.update_rollout(rollout_id=rollout.rollout_id, config=config)

    # Dequeue to create an attempt and add span to make it running
    attempted = await db_store.dequeue_rollout()
    assert attempted is not None
    await db_store.add_otel_span(rollout.rollout_id, attempted.attempt.attempt_id, mock_readable_span)

    # Verify it's running
    running_rollouts = await db_store.query_rollouts(status=["running"])
...
async def test_healthcheck_unresponsive_behavior(
    ...
    rollout = await db_store.enqueue_rollout(input={"test": "unresponsive"})
    await db_store.update_rollout(rollout_id=rollout.rollout_id, config=config)
...
@pytest.mark.asyncio
async def test_full_lifecycle_success(db_store: DatabaseLightningStore, mock_readable_span: Mock) -> None:
    """Test successful rollout lifecycle: queue -> prepare -> run -> succeed."""
    # 1. Create task
    rollout = await db_store.enqueue_rollout(input={"test": "data"}, mode="train")
    assert rollout.status == "queuing"
    ...
    span = await db_store.add_otel_span(rollout.rollout_id, attempt.attempt_id, mock_readable_span)
    ...
    # Verify final state
    final = (await db_store.query_rollouts())[0]
    assert final.status == "succeeded"
    assert final.end_time is not None

    final_attempt = await db_store.get_latest_attempt(rollout.rollout_id)
...
async def test_requeued_attempt_recovers_before_retry(
    ...
    attempted = await db_store.start_rollout(input={"foo": "bar"})
    await db_store.update_rollout(rollout_id=attempted.rollout_id, config=_retry_config())
    ...
    await db_store.add_otel_span(attempted.rollout_id, attempted.attempt.attempt_id, mock_readable_span)
...
async def test_requeued_attempt_recovers_after_retry_started(
    ...
    retried = await db_store.dequeue_rollout()
    ...
    first_attempts = await db_store.query_attempts(attempted.rollout_id)
