...
if __name__ == "__main__":
    ...
    train_jsonl_pathlist = []
    assert(os.path.exists(args.input_jsonl_path))
    for subdir_triplist in os.walk(args.input_jsonl_path):
        subdir_path, subdir_list, subdir_item = subdir_triplist
        if len(subdir_list) != 0:
            continue
        if len(subdir_item) == 0:
            continue
        for subdir_item_path in subdir_item:
            if subdir_item_path.endswith(".jsonl"):
                train_jsonl_pathlist.append(os.path.join(subdir_path, subdir_item_path))
    train_jsonl_pathlist = [jsonl_path for jsonl_path in train_jsonl_pathlist if os.path.exists(jsonl_path)]
